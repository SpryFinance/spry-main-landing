...
export default function BlackHole({ speedMultiplier = 1 }: BlackHoleProps) {
 ...
 const isMobile=useIsMobile()
  useEffect(() => {
    ...
    let rect: DOMRect
    let render: { width: number; height: number; dpi: number }
    let startDisc: Disc
    let endDisc: Disc
    let clip: { disc: Disc; i: number; path: Path2D }
    ...
    const tweenValue = (start: number, end: number, p: number, ease = "linear") => {
      const delta = end - start
      const easeFn = easingUtils[ease as keyof typeof easingUtils] || easingUtils.linear
    ...
    const setLines = () => {
      ...
      lines.forEach((line) => {
        ...
        line.forEach((p1, j) => {
          ...
          if (!lineIsIn && ctx.isPointInPath(clip.path, p1[0], p1[1])) {
            lineIsIn = true
          } else if (lineIsIn) {
  ...
  return (
    <div ref={containerRef} className="blackhole-container">
